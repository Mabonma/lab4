...
public:
	...
	//constructors
	Programming() { }
	Programming(string name_lang, string libr)
	{
		this->name_of_language = name_lang;
		this->library = libr;
	}

	//properties
	virtual string Name_of_language() = 0;
	virtual string Library() = 0;
...
public:
	...
	//properties
	string Name_of_language() override
	{
		return name_of_language;
	}

	string Library() override
	{
		return library;
	}
...
int main()
{
	...
	cout << "\n----------------------------------------------------------------\n" << endl;
	/*Programming* oops[3];
	oops[0] = &OOP(name, libr, inher, auto_encaps, polymorf);
	oops[0]->Print();
	...
	cout << "\n----------------------------------------------------------------\n" << endl;
	/*oops[1] = &Markup(name, libr, tag, framework);
	oops[1]->Print();
	...
	sctipt.Print();
	cout << sctipt.ToString() << endl;
	/*oops[2] = &Scripts(name, libr, target, type);
	oops[2]->Print();
